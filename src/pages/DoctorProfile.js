  ...
  const fetchProfile = async () => {
    try {
      ...
      setFormData({
        ...
        bookingCycleDays: data.user.bookingCycleDays || 1
      });
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Booking Cycle</label>
              <select
                value={formData.bookingCycleDays}
                onChange={(e) => setFormData({ ...formData, bookingCycleDays: parseInt(e.target.value) })}
                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
